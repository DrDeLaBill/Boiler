...
void ErrorService::check_failure(){
  ...
  if (this->is_set_error(ERROR_PUMPBROKEN)){ // check_pump()
...
void ErrorService::add_error(uint8_t new_error) {
  if (this->is_set_error(new_error)) {
    return;
  } else if (_type_error_validate(new_error)) {
...
bool ErrorService::_type_error_validate(uint8_t error_type) {
...
void ErrorService::init_error_actions() {
  for (uint8_t i=0; i < this->errors_list.size(); i++) {
    switch(this->errors_list.at(i)) {
      case ERROR_NOERROR:
        break;
